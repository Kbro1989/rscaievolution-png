...
class Textures extends SpriteArchive {
    ...
    addTextureSprite(name, indexData, spriteData) {
        ...
        this.sprites.set(name, this.parseSprite(spriteData, indexData, 1)[0]);
    }
    ...
    loadArchive(buffer) {
        ...
        for (const { name, subName } of this.textures) {
            if (name && name.length) {
                const spriteData = this.archive.getEntry(`${name}.dat`);
                this.addTextureSprite(name, indexData, spriteData);
            }

            if (subName && subName.length) {
                const spriteData = this.archive.getEntry(`${subName}.dat`);
